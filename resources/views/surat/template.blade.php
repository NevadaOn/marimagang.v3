...
<body>
    ...
    <main>
        ...
        <p style="line-height: 1.5;" class="text-center nomor">Nomor: {{ $nomorSurat ?? '[NOMOR SURAT]' }}</p>
<div class="content">

@php
    $totalOrang = 1 + ($pengajuan->anggota->count() ?? 0);
...
@endphp
    ...
    <p style="margin-top: 5px;">Dengan ini menerangkan bahwa:</p>
    <table style="line-height: {{ $lineHeight }};">
        @php $no = 1; @endphp

        <tr>
            <td style="width: 150px;">{{ $no++ }}. Nama</td>
            <td style="width: 10px;">:</td>
            <td>{{ $pengajuan->user->nama ?? '[NAMA MAHASISWA]' }}</td>
        </tr>
        <tr>
            <td style="padding-left: 15px;">NIM</td>
            <td>:</td>
        ...
        <tr>
            <td>Program Studi</td>
            <td>:</td>
            <td>{{ $pengajuan->user->universitas->prodi ?? '-' }}</td>
        </tr>
        <tr>
            <td>Fakultas / Jurusan</td>
            <td>:</td>
            <td>{{ $pengajuan->user->universitas->fakultas ?? '-' }}</td>
        </tr>
        <tr>
            <td>Asal Universitas</td>
            <td>:</td>
            <td>{{ $pengajuan->user->universitas->nama_universitas ?? '-' }}</td>
        </tr>
    </table>
        ...
        <div style="width: 100%; margin-top: 15px;">
            <div style="width: 40%; float: right; text-align: left;">
                <p style="margin-bottom: 10px;">Malang, {{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}</p>
